...
### Added
- **Emoji 短码语法**：支持 GitHub 短码全集；输入 `:` + 字母时弹出模糊匹配下拉框；`:shortcode:` 源码文本可编辑，移出光标自动重建。
- **应用自动更新**：设置 > 系统新增「检查更新」功能。

### Changed
- **大纲折叠按钮对齐文件树**：折叠按钮大小、可点击范围、悬停背景色与缩进距离与文件树统一。

## [0.7.8] — 2026-08-06

### Added
- **大文档打开加载遮罩**：打开 2000 行以上的大型 Markdown 文档时，编辑器区域立即显示「加载中」遮罩；加载完成后遮罩自动消失。

### Changed
- **大文档打开速度显著提升**：通过消除冗余的 Markdown 解析、跳过超长文档的格式规范化、将 Markdown 解析移入 Web Worker，文档的打开延迟大幅降低。
- **文件树深层目录展开不再卡顿**：打开包含深层级目录结构（如 `node_modules`）的工作区时，展开目录不再产生数百个无意义的后台文件系统调用；文件树采用虚拟滚动，仅渲染视口内可见行，无论展开多少目录都能流畅滚动。
- **侧栏拖拽更平滑**：拖拽侧栏分隔条调整宽度时，编辑器区域不再因布局重算而卡顿，拖拽过程视觉更流畅。
...
- **CJK 中文排版增强**：新增三套 CJK 排版功能，全面提升中文写作体验。
  ...
  - **括号自动配对**：键入 `（` `【` `「` `『` `《` `〈` 等开括号时自动插入对应闭括号，光标居中；支持闭括号跳越、成对删除、Tab 跳越。默认开启，可在设置中关闭。
  - **智能排版格式化**：新增「格式化 CJK」命令（支持全文 / 选区 / 当前块），自动处理中英文间距、全角标点、智能引号转换、破折号、省略号等排版规范。
  - **辅助输入与格式化规则细分开关**：设置页编辑器分组新增「辅助输入」与「格式化规则」多选下拉，可分别控制各排版子项的开关。
- **文件树目录头悬浮固定**：滚动文件树时，已滚出视口的目录行会固定在顶部，不再丢失层级上下文。
- **文件树根行快捷按钮**：悬停工作区根行时右侧显现新建文件、新建文件夹、刷新、全部折叠四个快捷按钮。

### Changed
- **界面配色统一**：活动栏、侧栏、编辑区、浮层等界面背景色统一管理，暗色模式下灰阶更协调。
- **活动栏选中指示器改为竖条**：选中项不再用背景色表达，改为一根主题色竖条贴左缘指示，视觉更克制。
- **空目录保留展开箭头**：空子目录不再隐藏展开箭头，无论空 / 非空都显示，交互更一致。


## [0.7.6] — 2026-07-29

### Added
- **大文档性能优化**：所见即所得模式（ProseMirror）打开 / 浏览 5000+ 行文档时的卡顿显著改善。
- **统一 Toast 通知系统**：保存成功 / 保存失败、导出完成、替换完成、外部修改检测等操作现在以右上角浮层 Toast 形式给出即时反馈，取代散落各处的原生弹框与静默成功；自动消失 + 手动关闭，支持成功 / 信息 / 警告 / 错误四种类型。

### Changed
- **macOS 标题栏不再显示文件名与应用名称**：macOS 原生标题栏（overlay 模式）此前会居中显示 `文件名 - Velo Editor` 文本，与自定义顶栏的菜单按钮和标签页重叠；现在标题栏文本已隐藏，交通灯按钮不受影响。Windows / Linux 不受影响。


## [0.7.5] — 2026-07-23

### Added
- **跟随系统深浅色模式**：暗色模式开关改为三态选择（跟随系统 / 始终浅色 / 始终暗色），「跟随系统」自动监测操作系统深浅色变化并实时切换；旧版本设置自动迁移。
- **主题色影响文档内容**：外观设置新增「主题色影响文档颜色」开关（默认关闭），开启后文档内的标题、加粗、链接、列表、折叠、表格、代码块头部等元素颜色跟随主题色；关闭时文档内容使用默认颜色，UI 界面始终跟随主题色。
- **代码块主题色板预览**：代码块主题选择新增色块预览，直观展示各主题的代表色（关键字 / 字符串 / 函数 / 注释）。
- **代码块「无主题」选项**：代码块主题选择新增「无主题」选项，选中后跳过语法高亮，代码块显示纯文本。

### Changed
- **设置页下拉框统一为 VeloSelect 组件**：设置面板中所有下拉选择框统一为自绘 VeloSelect 组件（浮层面板 + 键盘导航 + 主题色高亮选中项），取代浏览器原生下拉框。
- **拦截浏览器内置快捷键**：拦截 Ctrl+G、Ctrl+R、F3、F5 等浏览器内置快捷键，避免在桌面应用中误触刷新页面等浏览器行为。

### Fixed
- **启动时主题色闪烁**：修复开启「主题色影响文档颜色」后，启动时文档内容先显示默认黑色再跳变为主题色的问题。
- **启动时标签恢复顺序错乱与「未命名」Tab 闪烁**：修复重新打开 Velo 时，上次关闭的标签只显示激活标签、其他标签在内容加载后才出现、且激活标签位置跳到首位的问题；以及先显示一个「未命名」Tab 再变成上次标签的问题。现在所有标签按原始顺序同时显示，内容异步加载。
- **切换标签时滚动位置丢失**：修复切换标签再切回后，滚动位置未保留的问题。
- **折叠区段内代码块头部孤悬**：修复标题折叠包含代码块时，代码块内容被隐藏但头部工具条仍显示在折叠区段外的问题。


## [0.7.4] — 2026-07-22

### Added
- **上标与下标语法**：支持 `^x^` 上标和 `~x~` 下标；单 `~` 为下标、双 `~~` 仍为删除线；`Ctrl+Alt+加号` / `Ctrl+Alt+减号` 快捷键切换。
- **macOS 与 Linux 文件夹右键菜单**：「在 Velo 中打开」右键菜单从 Windows 扩展到 macOS（Finder 服务）和 Linux（KDE / GNOME 桌面环境）。
- **CLI 启动自动展开文件树**：通过命令行或右键菜单打开文件 / 文件夹时，侧栏自动切换到文件树视图；右键打开单个 Markdown 文件时自动取其所在目录作为工作区根。
- **`<hr>` HTML 标签解析**：单行 `<hr>` / `<hr />` 等 HTML 标签自动解析为分割线节点（同 `---` 语义）。

### Changed
- **macOS 原生窗口风格**：macOS 开启原生标题栏，交通灯按钮浮在自定义标题栏左上角；Windows / Linux 不受影响。
- **侧栏分隔线改为阴影渐变**：侧栏与编辑器之间的硬分割线改为柔和的阴影渐变，视觉更简洁；拖拽时仍显示主题色高亮抓取线。

### Fixed
- **CJK 标点导致加粗失效**：修复含全角标点（`：` `（` `）` `。` `，`）的加粗文本被解析为字面量 `**` 而非加粗格式的问题。
- **列表下方空段落 Backspace 扩展列表**：修复在列表下方的空段落按 Backspace 会反向扩展列表而非直接删除空行的问题。


## [0.7.3] — 2026-07-20

### Added
- **设置页搜索**：设置页顶部 Tab 栏右侧新增搜索框，输入关键词即可跨所有设置分组模糊匹配，点击结果自动跳转到对应分组并高亮目标设置项；新增设置项时无需额外适配，自动纳入搜索索引。
- **空页面内联欢迎界面**：没有打开任何文档时，编辑器区域直接显示新建 / 打开文件入口，不再弹出独立对话框。

### Changed
- **设置面板改为整页接管编辑器主区域**：设置从原左侧栏窄面板升级为整页 Tab 切换，顶部 Tab 栏切换类目，设置 Tab 可后台保留、切文档不消失。
- **文件菜单移至顶栏左侧**：原 ActivityBar 顶部的文件下拉菜单移到顶栏左侧向下箭头按钮，点击在按钮正下方展开。
- **工作区面板浮动滚动条**：文件树 / 大纲 / 搜索等侧栏面板改用浮动 overlay 滚动条，不占用内容宽度。
- **设置项选择框改为同行布局**：代码块主题、启动时打开等下拉选择框与标题改为同一行显示，选择框保持固定宽度。

### Fixed
- 设置页 / 空页面时文件菜单不再显示保存 / 另存为 / 导出。
