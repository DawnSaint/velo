...
- **mark 语法闭合后必须 `removeStoredMark`**: `**bold**` / `*it*` / `==hl==` / `~~st~~` / `_em_` 的 apply(`delete → insertText → addMark`)后,光标停在 inner 末尾 = inclusive mark 右边界。这些 mark 的 `inclusive` 默认 true(schema 未设,与 `link` 的 `inclusive:false` 不同),且 apply 未设 storedMarks(=null),继续输入时 PM 回退到 `$from.marks()`(含该 mark)→ 继承粗体/高亮。修复:apply 末尾 `tr.removeStoredMark(markType)` 让 storedMarks=[] 覆盖继承(storedMarks 优先于 `$from.marks()`,故 inclusive 边界也能退出;与 Typora/Obsidian 一致)。**不要**改 `inclusive:false` 解决 —— 会破坏 Ctrl+B 连续输入(`markCommands` 的 `addStoredMark` 首字符消耗后,后续字符靠 inclusive 边界继承);`link` 能用 `inclusive:false` 因它不靠 Ctrl+B 连续输入。**这只覆盖"语法刚闭合那一笔"**;光标进入已有 mark 范围继续编辑由 `markSourceEditPlugin` 处理(见下条),两者互补不重叠
- **mark 源码编辑 session(Obsidian Live Preview 风格)**: `markSourceEditPlugin` 让光标进入 `**bold**` / `*it*` / `==hl==` / `~~st~~` 范围时,整段换成可编辑的 markdown 源码字符;移出光标 commit 还原成 mark。session 状态机(apply mapping bias +1/-1 + pendingCommit + `view().update` 触发 commit + Escape 还原)逐字照搬 `linkClick.ts` / `imageEditPlugin.ts`。
  ...
  - **`buildMarkSource` 自己拼保 `*`/`_`**:不能用 `toMarkdown` —— 它把 emphasis 恒输出 `_`、strong 恒 `**`,丢 `attrs.marker`。遍历 slice inline 节点按固定嵌套序(开:strong→emphasis→strike→highlight;闭反序)在 mark 转换处插 `markerText(mark)`(读 `attrs.marker`)。`**bold *italic***` → 两端 `**`+`*` 全显;`__bold__`/`_it_` 原样
  ...
  - **退避三处**:`syntaxAutoFormat.getActiveEditRange` 加 `markSourceEditKey.session` 分支(防编辑源码时被 `strongSyntax` 又转回 mark);`markCommands.forceStoredMarkOnly` 扩展含 markEditSession(session 内 Mod-b 只 `setStoredMark` 不插 `****`);link/image session 范围内不进(session 互斥,`buildMarkSource` 不处理 link 语法)
  ...
  - 装入位置:`syntaxAutoFormatPlugin` **之前**(`EditorInner.vue` 的 `allPlugins`)—— appendTransaction 的 enter 让 `getActiveEditRange` 在 pass 2 读到 session 自动退避。与 link/image escape keymap 同区;三者 Escape 各自 session 为 null 时 return false,顺序非关键
