...
- **NodeView 隔离**: `ignoreMutation()` + `stopPropagation` 隔离 ProseMirror。**坑**:PM 的 `view.dom` 是 contentEditable 容器,所有 selection 统一由 PM 管理;在 NodeView 里嵌套 `contentEditable=true` 的子元素(sup / labelSpan)拿不到独立 focus —— 浏览器把 focus 给了子元素但 PM 的 selection 仍停在外层,Backspace/Delete 按 PM 的 selection(子元素外)处理 → "删错位置"。需要"光标进入、逐字符编辑"的 inline 节点(如 `footnote_reference`)不要走 atom + contentEditable,改成 `content: 'text*'` 的非 atom 节点 + `contentDOM = dom`,让 PM 直接接管 text 编辑(selection 自然进入)。label 作为 text content 而非 `attrs.label`,markdownIO 双向适配。`footnote_definition` 的 label 仍在 `attrs.label`(label 不在 contentDOM 子树内,后续如需 Typora 式编辑可把 label 做成 content 的一部分)
