...
- **macOS 26 (Tahoe) ObjC 异常兼容**: macOS 26 修改了 ObjC 异常处理行为,之前能被静默忽略的 ObjC 异常现在会穿越 `extern "C"` 边界,触发 `panic_cannot_unwind` → `abort()`(tao#1171, tauri#15517)。`finder_service.rs` 中所有 `msg_send!` 调用用 `objc_exception::r#try` 包裹,捕获后降级为 warn 日志。**注意**: objc 0.2.7 的 `exception` 模块是私有的(`mod exception;` 而非 `pub mod`),即使启了 `exception` feature 也无法从外部访问;必须直接依赖 `objc_exception` crate(0.1.x)使用其 `r#try` 函数(`try` 是 Rust 2018+ 保留关键字,需 `r#` 转义)。objc crate 的 `class!` / `msg_send!` / `sel!` 宏内部使用已废弃的 `cfg(cargo-clippy)`,会触发 `unexpected_cfgs` 警告,已在 `finder_service.rs` 模块级别 `#![allow(unexpected_cfgs)]` 抑制
