// WYSIWYG code_block 行号 plugin 测试(v0.5.11)。
//
// 测例覆盖主要路径:
// 1. enabled=false → 无 .velo-code-gutter-widget DOM
// 2. enabled=true + code_block 3 行 → 1 个 widget,内含 3 个 .velo-code-gutter-line
// 3. 切 lang 不影响行号(行号数 = 行数,与 lang 无关)
// 4. mermaid code_block → 无 widget
// 5. 空 code_block(无内容)→ 无 widget
// 6. 跨多个 code_block → 每个独立 widget,key 区分 pos
// 7. docChanged 在 code_block 内加 1 行 → 重建 widget(行数从 3 变 4)
// 8. tr.setMeta(lineNumbersKey, { enabled: false }) → widget 消失
...
describe('codeLineNumberPlugin', () => {
  it('1. enabled=false → 无 .velo-code-gutter-widget DOM', () => {
    const view = makeView('```js\nconst x = 1\nconst y = 2\n```', false)
    // 等一帧让 widget factory 跑完(rAF 同步位置)
    expect(view.dom.querySelector('.velo-code-gutter-widget')).toBeNull()
    view.destroy()
  })

  it('2. enabled=true + code_block 3 行 → 1 个 widget,内含 3 个 .velo-code-gutter-line,文本 1/2/3', () => {
    const view = makeView('```js\nconst x = 1\nconst y = 2\nconst z = 3\n```', true)
    const widget = view.dom.querySelector('.velo-code-gutter-widget')
    expect(widget).not.toBeNull()
    const lines = widget!.querySelectorAll('.velo-code-gutter-line')
  ...
  it('3. 切 lang 不影响行号(行号数 = 行数,与 lang 无关)', () => {
    ...
    expect(view.dom.querySelectorAll('.velo-code-gutter-line').length).toBe(3)
  ...
  it('4. mermaid code_block → 无 widget', () => {
    const view = makeView('```mermaid\ngraph TD\n  A --> B\n  C --> D\n```', true)
    expect(view.dom.querySelector('.velo-code-gutter-widget')).toBeNull()
    view.destroy()
  })

  it('5. 空 code_block(无内容)→ 无 widget', () => {
  ...
  it('6. 跨多个 code_block → 每个独立 widget,行号分别从 1 开始', () => {
    const md = '```js\nconst a = 1\n```\n\n```py\nx = 1\nx = 2\nx = 3\n```'
    const view = makeView(md, true)
    const widgets = view.dom.querySelectorAll('.velo-code-gutter-widget')
    expect(widgets.length).toBe(2)
    // 第一个:1 行 → 1 个行号
    expect(widgets[0]?.querySelectorAll('.velo-code-gutter-line').length).toBe(1)
    // 第二个:3 行 → 3 个行号
    expect(widgets[1]?.querySelectorAll('.velo-code-gutter-line').length).toBe(3)
    view.destroy()
  })
  ...
  it('7. docChanged 在 code_block 内加 1 行 → 重建 widget(行数从 3 变 4)', () => {
    ...
    expect(view.dom.querySelectorAll('.velo-code-gutter-line').length).toBe(4)
    view.destroy()
  })

  it('8. tr.setMeta(lineNumbersKey, { enabled: false }) → widget 消失', () => {
    const view = makeView('```js\nconst x = 1\n```', true)
  ...
  it('9. dark 模式 <html class="dark"> → 行号颜色由 var(--shiki-light) 翻面到 var(--shiki-dark)', () => {
    ...
    // widget 自身 inline style 不应包含 color 字段,翻面由 SCSS 接管。
    const view = makeView('```js\nconst x = 1\n```', true)
    const widget = view.dom.querySelector('.velo-code-gutter-widget') as HTMLElement | null
    expect(widget).not.toBeNull()
    // widget JS 设置的 inline style 只有 position / zIndex / pointerEvents
    // (见 makeGutterDom 工厂);颜色由 SCSS 接管,inline style 不写 color。
    const inline = widget!.getAttribute('style') || ''
