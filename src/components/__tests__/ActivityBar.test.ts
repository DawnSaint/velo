...
describe('ActivityBar', () => {
  ...
  it('marks the active action as pressed', () => {
    const wrapper = mount(ActivityBar, { props: { active: 'settings' } })

    expect(wrapper.get('[aria-label="设置"]').attributes('aria-pressed')).toBe('true')
    expect(wrapper.get('[aria-label="文件"]').attributes('aria-pressed')).toBe('false')
  })
})
