...
describe('editor store ActivityBar 自定义 (v0.6.1)', () => {
  ...
  it('默认顺序 files/outline/search,无隐藏,visible 即 order', () => {
    const store = useEditorStore()
    expect(store.activityBarOrder).toEqual(['files', 'outline', 'search'])
    expect(store.activityBarHidden).toEqual([])
    expect(store.visibleActivityBarItems).toEqual(['files', 'outline', 'search'])
  })

  it('reorder before: 把 from 移到 to 之前', () => {
    const store = useEditorStore()
    store.reorderActivityBar('outline', 'files', 'before')
    expect(store.activityBarOrder).toEqual(['outline', 'files', 'search'])
  })

  it('reorder after: 把 from 移到 to 之后(跨项)', () => {
    const store = useEditorStore()
    store.reorderActivityBar('files', 'search', 'after')
    expect(store.activityBarOrder).toEqual(['outline', 'search', 'files'])
  })
  ...
  it('toggleActivityBarHidden 来回切换,visible 同步过滤', () => {
    ...
    expect(store.visibleActivityBarItems).toEqual(['files', 'search'])
    store.toggleActivityBarHidden('outline')
    expect(store.activityBarHidden).toEqual([])
    expect(store.visibleActivityBarItems).toEqual(['files', 'outline', 'search'])
  })
  ...
  it('hydrateActivityBarConfig 走 normalize 写入 store(settings 从 hidden 剔除)', () => {
    ...
    expect(store.activityBarOrder).toEqual(['search', 'files', 'outline'])
    expect(store.activityBarHidden).toEqual(['outline'])
  })
})
...
describe('normalizeActivityBarConfig 防御性归一化', () => {
  it('过滤未知项 + 按默认序补齐缺失项', () => {
    ...
    expect(order).toEqual(['search', 'files', 'outline'])
    expect(hidden).toEqual([])
  })

  it('dedupe order 重复项', () => {
    const { order } = normalizeActivityBarConfig(['files', 'files', 'search'], [])
    expect(order).toEqual(['files', 'search', 'outline'])
  })
  ...
  it('undefined 输入回退默认', () => {
    ...
    expect(order).toEqual(['files', 'outline', 'search'])
    expect(hidden).toEqual([])
  })
